...
**Сокет** - комбинация из IP-адреса и порта.   
...
Для преобразования доменного имени в IP браузер и ОС сначала используют локальные и настроенные источники, где ответ уже может быть известен:
1. Кэш браузера
2. Кэш ОС
3. Файл hosts
4. Настроенный DNS-resolver {DNS провайдера / Облачный DNS (например Cloudflare) / Google DNS (8.8.8.8 или 8.8.4.4)}  
...
**TLS Handshake** - TLS использует гибридную схему.
Сертификат и ассиметричная криптография используются для аутентификации сервера и согласования общего секрета/ключей.  
Последующая передача данных осуществляется с помощью симметричного шифрования.  
...
**Certificate Check** - установка защищенного соединения  
...
На клиенте происходит проверка сертификата: подписей удостоверяющих центров, проверка доверия удостоверяющему центру (совпадает ли с разрешенными на стороне клиента), проверка домена сертификата (на случай подмены), проверка срока действия сертификата, проверка не отозван ли сертификат и т.д.

**Key Exchange**  - в TLS 1.2 и более старых схемах параметры для вычисления общего секрета могут передаваться по-разному в зависимости от выбранного алгоритма.  
В старых RSA-схемах клиент генерирует `pre-master secret`, шифрует его публичным ключом сервера и отправляет серверу.  
В схемах на основе Diffie-Hellman общий секрет вычисляется обеими сторонами независимо.  
* `Client --- Client Key Exchange --> Server` - Данные из которых обе стороны смогут вычислить общий секрет / сеансовые ключи. Содержимое сообщения зависит от выбранного алгоритма.   
...
В TLS 1.2 на основе `pre-master secret`, `client random` и `server random` с обеих сторон генерируются ключи для симметричного шифрования и MAC.

В TLS 1.2 для возобновления соединения может использоваться идентификатор сессии или другие механизмы resumption.
...
Для корректного завершения TLS используются alert-сообщения `close_notify`. После обмена ими стороны завершают защищенное соединение.  
Это отдельный механизм поверх TCP, поэтому завершение TLS и завершение TCP-соединения логически различаются.  
...
* `Client --- Client Hello, Client key_share --> Server` - Вместе с `Client Hello` клиент отправляет `key_share` для одной или нескольких поддерживаемых групп, чтобы сократить число обменов.  
...
Не нужно ожидать отправки предыдущего запроса при работе в одном соединении (на уровне HTTP-потоков решается проблема `Head-of-line Blocking`, но на уровне TCP она остается)  
...
* **Начало навигации**  
Когда пользователь нажимает `Enter`, UI thread инициирует навигацию, а network-thread начинает сетевой запрос (происходит DNS Lookup, TCP/TLS Handshake)  
...
DOM - описывает содержимое документа в древовидно структуре.   
...
Скрипты могут блокировать парсинг до выполнения кода скрипта, т.к. JS может изменить форму документа (значит и сам DOM).  
  **НО** браузер продолжает синтаксический анализ html после запуска скрипта и при наличии дальше ссылок на внешние ресурсы через `preload scanner` начинает их загрузку
...
**Создание AOM (Дерево доступности)** - дерево для вспомогательных технологий (скринридеров и т.д.), которое строится на основе DOM и вычисленных свойств элементов.  
