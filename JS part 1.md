...
Неявное преобразование `variable` в Boolean `if(variable) {}`, где значения: `0, -0, '', null, undefined, NaN, 0n (BigInt)` эквивалентны логическому `false`  
...
Рекомендуется всегда указывать `defalt` для исключения возможных ошибок при расширении кода    
```javascript
switch(condition) {
...
К итерируемым сущностям относятнся: массив, строка, NodeList, HTMLCollection, Map, Set, генератор, arguments функции и т.д.    
Обход осуществляется по значениям элементов, для одновременного доступа к паре ключ-значение используется деструктуризация:  
...
* Примитивные (`string, number, bigint, boolean, null, undefined`) - в переменную записывается значение 
...
Символ создает скрытые свойства в объекте, которые не выводиятся через `Object.keys(obj)`, for..in, но к ним можно обратиться по имени:      
```javascript
let id = Symbol('id') 
...
>null **строго равен ТОЛЬКО undefined и самому себе.** Аналогично для undefined: `null == undefined // true`  
...
`this` вложенныъ обычных/анонимных функций **не привязан** к родительскому объекту (можно явно задать контекст через `bind`)  
...
* `array.toSpliced(start[, deleteCount[, item1[, item2[, ...]]]])` - аналог `splice`. Возвращает **новый измененный массив**  
>При вызове метода преобразует все пустые элементы в undefined
...
* `arr.toSorted(compareFunction(a,b){})` - аналог `sort`. Возвращает **копию исходного массива**  
* `array.reverse()` - возвращает **исходный массив** с обратным порядком следования элементов  
* `array.toReversed()` - аналог `reverse`. Возвращает **копию исходного массива**  
...
Для этого нужно задать `Symbol.iterator` объекту и вернуть из него метод `next()`, который возвращает объект со свойствами `value` и `done`, где 
`value` - перебираемое значение, `done` - статус выполнения перебора.  
```javascript
const obj = {  
	name: 'Alex',  
	age: 18,  
...
Вариант с функцей-конструктором:  
```javascript
...
Метод `throw()` - завершает выполнение функции и выбрасывает ошибку  
...
* Конвертации псевдомассивов (NodeList / HTMLCollection) в массив: `[...nodeList]`  
