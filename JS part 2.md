...
* [2. Web API](#2-web-api) 
	- [2.1 History API / Navigation API](#21-history-api--navigation-api)  
	- [2.2 Location API](#22-location-api)   
	- [2.3 Navigator API](#23-navigator-api)   
	- [2.4 Trusted Types API](#24-trusted-types-api)   
	- [2.5 Clipboard API](#25-clipboard-api)   
	- [2.6 URLPattern API](#26-urlpattern-api)   
...
DOM - древовидная структура для управления HTML-разметкой из JS-кода. Формируется в `document` при разборе HTML, а `DOMContentLoaded` срабатывает после того, как DOM построен  
...
_Живые коллекции/элементы обновляет свой состав при изменении DOM: если подходящие элементы добавить или удалить после получения коллекции, переменная с коллекцией начнет отражать новое состояние DOM._   
_Неживые(статические) коллекции/элементы не обновляет свой состав после получения: это снимок найденных элементов на момент вызова метода._  
...
* `Node.getRootNode()` - возвращает корневой узел (`Document`, `ShadowRoot`)
...
>В `append, prepend, after, before` можно передавать сам узел или строку: `before(some_id)` и `before('#some_id')`
...
* `Element.insertAdjacentHTML(position, HTMLtext)` - вставляет HTML-текст в позицию (beforebegin, afterbegin, beforeend, afterend)
...
>Методы `innerHTML`, `outerHTML` работают со строкой из которой создаются элементы  
...
* `Element.style.cssText` - возвращает/устанавливает свойства с полным перезаписыванием атрибута `style`. **Вставляются инлайново**
* `Element.style.backgroundColor` - возвращает/устанавливает конкретное свойство. **Вставляются инлайново**.  
...
* `Element.clientLeft`, `Element.clientTop` - ширина левой / верхней границы элемента
* `Element.getClientRects()` - возвращает коллекцию прямоугольников элемента (`DOMRectList`) с информацией о размерах и положении
inline элементы могут иметь не строго прямоугольные границы, если текст переносится на другие строчки
* `Element.getBoundingClientRect()` - возвращает DOMRect с информацией о размерах элемента по охватывающему его прямоугольнику (width, height, x, y, top, right, bottom, left)
...
* `document.forms, document.links, document.images` - возвращает HTMLCollection всех (`<form>`/`<area> и <a>`/`<img>`) в документе
...
* `document.getSelection()` - возвращает объект `Selection` с информацией о выделении
...
* `Node.normalize()` - удаляет пустые текстовые узлы и объединяет соседние текстовые узлы
...
2. `on-свойство` - (`onclick`, `onload` и т.д.). Передача null в свойство удаляет обработчик   
...
Также в `handler` передается объект Event, обладающий свойствами (различными в зависимости от типа события):
...
* `event.stopPropagation()` - блокирует дальнейшее распространение события по DOM (в том числе может использоваться в Capture-фазе)  
...
* `scrollend` - событие срабатывает при прокрутке страницы/элемента до конца <font color="#7ead74">_**[Baseline 2025]**_</font>  
...
### 2.1 History API / Navigation API
...
* `History.forward()` - перемещение на страницу вперед (равносильно go(1))
...
Navigation API - современный Web API для работы с навигацией страницы и history stack. (замена History API) <font color="#7ead74">_**[Baseline 2026]**_</font>  
...
Основные методы:
* `Navigation.navigate(url)` - переход к новому адресу
* `Navigation.back()` - назад по истории
* `Navigation.forward()` - вперед по истории
* `Navigation.reload()` - перезагрузка текущей записи
* `Navigation.traverseTo(key)` - переход к конкретной записи history по ключу
...
* `Location.replace(url)` - аналогично assign, но текущая страница **НЕ сохраняется** в истории  
...
`window.navigator` - объект с информацией о браузере и среде _разобран частично_
...
* `Navigator.storage` - проверки quota и persistent storage, доступного объема хранилища и запроса persistent storage, чтобы браузер не удалял данные сайта при нехватке места. <font color="#7ead74">_**[Baseline 2026]**_</font>   
...
### 2.4 Trusted Types API

Trusted Types API <font color="#7ead74">_**[Baseline 2026]**_</font> - механизм защиты от DOM-based XSS, который при включенном CSP (`require-trusted-types-for`) запрещает передавать обычные строки в опасные DOM sink-места (`innerHTML`, `outerHTML`, `insertAdjacentHTML`, `script.src` и т.д.)  
...
`Clipboard API` - Web API для чтения и записи данных в системный буфер обмена через `navigator.clipboard`. <font color="#7ead74">_**[Baseline 2024]**_</font>   

Методы (возвращают `Promise`):
* `navigator.clipboard.readText()` - читает строку из буфера обмена
* `navigator.clipboard.writeText(newClipText)` - записывает строку в буфер обмена
* `navigator.clipboard.read(formats?)` - читает clipboard-данные как массив [ClipboardItem](https://developer.mozilla.org/en-US/docs/Web/API/ClipboardItem) (например, HTML, изображения и другие форматы), где  
formats - опциональный параметр MIME типов, которые не должны проходить санитайз при чтении буфера
* `navigator.clipboard.write(data)` - записывает массив [ClipboardItem](https://developer.mozilla.org/en-US/docs/Web/API/ClipboardItem) в буфер обмена  
 
* `ClipboardItem.supports(type)` - метод для проверки, поддерживает ли браузер указанный MIME-тип в `ClipboardItem`. <font color="#7ead74">_**[Baseline 2025]**_</font>

[Вернуться к содержанию](#содержание)

### 2.6 URLPattern API

`URLPattern` - API для сопоставления URL по отдельным частям (`protocol`, `hostname`, `pathname`, `search`, `hash` и т.д.) с поддержкой именованных параметров и шаблонов. <font color="#7ead74">_**[Baseline 2025]**_</font>  
...
Устанавливается в событии `dragstart`. В других событиях не оказывает эффекта
	...
	* linkMove
	* all
* `DataTransfer.dropEffect` - свойство определяющее тип операции в drop области
Устанавливается в событии `dragover`. Возможные значения: `none`, `copy`, `move`, `link`
...
RegExp - регулярное выражение, представляющее собой шаблон для поиска и замены текста, проверки соответствий строки шаблону  
Создание:
1. `new RegExp('pattern', 'flags')` - можно использовать шаблонные строки с переменными  
2. `/pattern/flags`
`pattern` - регулярное выражение  
`flags` - необязательные флаги для регулярного выражения   
...
* `String.matchAll(regexp)` - работает только с флагом `g`. Возвращает RegExp String Iterator (массивоподобную сущность) с совпадением и capture group  
Для `/([A-Z])([0-9])/g` в строке `'abCDA1234'` одно из совпадений будет `['A1', 'A', '1']`  
...
* `RegExp.escape()` - метод для экранирования строки перед вставкой в регулярное выражение. <font color="#7ead74">_**[Baseline 2025]**_</font>  
...
* `^` - НЕ - знак исключения, если записан в начале скобок `[]`
...
```
* `(?:pattern)` - НЕ группа захвата 

**Дополнение:**  
```javascript
const text = 'Some "text" in quotes "here"'
const regexp1 = /".+"/g
const matches1 = text.match(regexp1);
console.log(matches1); // ["\"text\" in quotes \"here\""]
// Для исправления и поиска текста в кавычках: 
const regexp2 = /".+?"/g
const matches2 = text.match(regexp2);
console.log(matches2); // ["\"text\"", "\"here\""]
```
...
|getOwnPropertyDescriptor|getOwnPropertyDescriptor|при получении дескриптора свойства `Object.getOwnPropertyDescriptor()`|
|defineProperty|defineProperty|при определении/изменении свойства `Object.defineProperty()`|
|has|has|проверка существования свойства `in`|
|get|get|чтение свойства|
|set|set|запись свойства|
|deleteProperty|deleteProperty|удаление свойства `delete`|
|ownKeys|ownKeys|при перечислении собственных ключей объекта `Object.keys, Object.getOwnPropertyNames, Object.getOwnPropertySymbols, Reflect.ownKeys`|
...
Object.defineProperties(target, {
    ...
    accessorProp: {
        ...
        set: undefined, // атрибут [[Set]] равен undefined
        configurable: false // неконфигурируемое
    }
});
const proxy = new Proxy(target, {
...
console.log(proxy.immutableProp); // Petr
proxy.immutableProp = 21; // попытка изменения значения неизменяемого и неконфигурируемого свойства
console.log(proxy.immutableProp); // Petr
...
```
* **Ловушка `deleteProperty(target, property)`** - должна возвращать boolean.  
...
Свойство не может быть объявлено неконфигурируемым, если в целевом объекте нет соответствующего неконфигурируемого собственного свойства.  
Свойство не может быть объявлено одновременно неконфигурируемым и неизменяемым, если в целевом объекте нет такого же собственного свойства.  
...
При обращении к нему свойство ищется в прототипе (`proxyWithoutReflect`), где срабатывает ловушка и возвращается значение из объекта `target`  
...
let proxyWithoutReflect = new Proxy(target, {
  get(target, prop, receiver) {
    return target[prop];
  }
});
...
let person1 = {
  __proto__: proxyWithoutReflect,
  _name: "Olga"
};
let person2 = {
  __proto__: proxyWithReflect,
  _name: "Olga"
};
...
>	}
>})
>proxyMap.set('name', 'Petr'); // работает
>```
...
`instanceof` - оператор проверяет является ли объект экземпляром конструктора или класса (просматривая цепочку прототипов)  
```javascript
const arr = [];
...
console.log(warrior.planet) // Mars
```
>Также стоит отметить, что стрелочные функции не имеют собственного `this`
...
// person.#role // SyntaxError: приватное поле недоступно вне класса
```
...
2. XMLHttpRequest 
3. FetchAPI
4. Экспериментальная технология: [showSaveFilePicker](https://developer.mozilla.org/en-US/docs/Web/API/Window/showSaveFilePicker#examples)
