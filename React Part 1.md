...
```
{
	stateNode, // ссылка на экземпляр компонента (для функциональный - null, классовых - экземпляр класса, DOM-узел - ссылка на DOM-элемент)
...
1. разные типа элементов (span => div) относятся к разным деревьям (изменении позиции элементов приводит к ререндеру) 
...
functin Parent(isVisible: boolean) {
	if(isVisible) {
		return <>
			<OptionalComponent />
			<ChildComponent />
