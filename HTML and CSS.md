...
* `<search>` - блок поиска/фильтра, результат поиска/фильтра <mark>Baseline 2023 </mark>  
...
* `<link>` - подключение внешних ресурсов (стили, шрифты, favicon и т.д.)  
  ...
  В отличие от обычного `preload`, браузер знает, что грузится именно ES module, и корректнее подготавливает его к выполнению. <mark>Baseline 2023 </mark>  
...
* `<iframe>` - встраиваемый элемент из стороннего источника (google maps, youtube видео)
	...
	* `loading` - ленивая загрузка <mark>Baseline 2023 </mark>  
...
Поддержка `<source>` для `<video>` и `<audio>`. [Source](https://scottjehl.com/posts/using-responsive-video/) <mark>Baseline 2023 </mark>  
...
Если нескольким элементам задан одинаковый `name` - будет оставлен открытым только один блок <mark>Baseline 2024 </mark>  
```html
...
* `inert` - атрибут, отключающий интерактивность элемента и всего его subtree. [MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/inert) <mark>Baseline 2023 </mark>  
...
Через ключевое слово `from` можно создавать новый цвет на основе другого цвета, получая доступ к его каналам (`r`, `g`, `b`, `h`, `s`, `l`, `alpha` и т.д.) и изменяя их. <mark>Baseline 2024 </mark>  

[Примеры](https://developer.chrome.com/blog/css-relative-color-syntax)

Добавлена поддержка новых цветовых пространств и функций в `color()` / `color-mix()`. <mark>Baseline 2023 </mark>  
```css
...
* cqw, cqh, cqi, cqb, cqmin, cqmax - единицы измерений, зависящие от размеров контейнера (`@container`) <mark>Baseline 2023 </mark>  
* lh - мультипликатор, зависящий от `line-height` <mark>Baseline 2023 </mark>  
```css
/* Было */
--line-height: 1.5;
...
* `child-група` - стиль элементов по порядковому номеру внутри родительского
	...
	* `:nth-last-child(n)` - n-ные дочерний элемент с конца (разрешены формулы с целочисленным результатом)  
    Микросинтаксис формул: `:nth-child(An+B [of S]?)`: <mark>Baseline 2023 </mark>  
...
* `:has()` - стиль элемент в зависимости от его потомков или содержимого <mark>Baseline 2023 </mark>   
...
* `:user-valid, :user-invalid` - аналог `:invalid, :valid` с решенной проблемой применения псевдокласса до начала взаимодействия пользователя с элементом <mark>Baseline 2023 </mark>
...
* `:lang(), :dir()` - стиль элементов по языку/направлению текста (`:dir` <mark>Baseline 2023 </mark>)
...
* `::backdrop` - псевдоэлемент положки под элементами всплывающими на верхний слой (`<dialog>, <popover>, <video> в полноэкранном режиме`
  `::target-text` - псевдоэлемент для стилизации текста, к которому браузер проскроллил страницу через text fragments (`#:~:text=...`) <mark>Baseline 2024 </mark>  
...
Свойство `display` поддерживает комбинацию двух значений (`outer inner`). <mark>Baseline 2023 </mark>  
...
Свойства позволяющие работать с дирктивой [@container](#211-директивы) <mark>Baseline 2023 </mark>  
...
* тригонометрические функции `sin(), cos(), tan(), asin(), acos(), atan(), atan2()` <mark>Baseline 2023 </mark>
---
...
* `font-size-adjust` - подстраивает размер fallback-шрифта так, чтобы он выглядел ближе к основному шрифту по выбранной метрике <mark>Baseline 2024 </mark>  
...
`grid-template-columns: subgrid` и/или `grid-template-rows: subgrid` - наследование размеров родительского grid в дочерних `display:grid` элементах <mark>Baseline 2023 </mark>  
...
В условиях с числовыми значениями можно использовать операторы сравнения: <mark>Baseline 2023 </mark>  
...
* `@container` - условные контрукции в стилях относительно размера контейнера <font color="blueocean">[Baseline 2023]</font>  
...
* `@starting-style` - директива для анимирования entry transitions элемента перед его появлением/рендером, в том числе для дискретных свойств (`display`, `visibility`, `mix-blend-mode` и аналогов). <mark>Baseline 2024 </mark>  
...
* `scrollbar-width` - ширина скролл-бара (`auto`, `thin`, `none`) <mark>Baseline 2024 </mark>.  
...
* `scrollbar-gutter` - позволяет зарезервировать место для скролбара, при его отсутствии, чтобы избежать сдвига макета <mark>Baseline 2024 </mark>  
...
* `counter-set` - изменение текущего значения счетчика на указанную: `counter-set: timerName 10` <mark>Baseline 2023 </mark>  
Если указан несуществующий счетчик, то работает как `counter-reset` <mark>Baseline 2023 </mark>
...
`@counter-style` - правило для стилизация счетчиков <mark>Baseline 2023 </mark>  
