...
* `<header>` - начальное/вводное содержимое страницы/блока
* `<footer>` - нижняя часть страницы/блока
* `<main>` - основное содержимое страницы. Может быть только 1 на странице  
_Внутри него не размещают `<header>` и `<footer>` страницы, но они могут быть внутри `<section>` блоков_
...
_Обычно внутри есть заголовок `<h.>`, чтобы раздел имел смысловую подпись_
...
_Нельзя вкладывать параграфы с текстом, заголовки, секционные блоки (`<article>, <aside>, <nav>, <section>`)_
* `<small>` - для копирайта, информации о лицензии, правовой информации, пояснений и комментариев
* `<search>` - блок поиска/фильтра, результат поиска/фильтра <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
* `<link>` - подключение внешних ресурсов (стили, шрифты, favicon и т.д.)  
    ...
    * `rel` - тип (`stylesheet/icon/apple-touch-icon-precomposed` и другие).  
    `modulepreload` - предзагрузка модулей. `<link rel="modulepreload" href="/app.js">`  
  В отличие от обычного `preload`, браузер знает, что грузится именно ES module, и корректнее подготавливает его к выполнению. <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
* `<audio>` - добавление аудиоплеера
	* `autoplay` - автовоспроизведение при загрузке. **В браузерах жестко ограничено** и часто требует `muted` или взаимодействия пользователя
...
* `<video>` - добавление видеоплеера
	...
	* `playsinline` - позволяет воспроизводить видео прямо внутри страницы на мобильных устройствах, а не переводить его автоматически в fullscreen
...
* `<iframe>` - встраиваемый элемент из стороннего источника (google maps, youtube видео)
	...
	* `loading` - ленивая загрузка <font color="#7ead74">_**[Baseline 2023]**_</font>  
* `<source>` - добавляет несколько источников видео/аудио/изображений для совместимости  
Вместо дублирования медиафайлов в `<source>` указывается src, srcset, sizes, media, type - MIME-тип  
`<source>` указываются в порядке приоритета типа (webm -> mp4)  
Поддержка `<source>` для `<video>` и `<audio>`. [Source](https://scottjehl.com/posts/using-responsive-video/) <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
```
При расположении один под другим используется подход (изображения вне таргета выключаются в стилях): 
```javascript
<svg xmlns="http://www.w3.org/2000/svg">
	<defs>
		<style>
...
* `<form>` - элемент формы. По умолчанию данные с формы отправляются синхронно, перезагружая страницу. Для изменения поведения часто используется JS
...
* `<input>` - поле ввода  
  ...
  Альтернативно можно задавать тип ввода в атрибуте `inputmode`
	...
	* `value` - изменяется в зависимости от типа (submit, button, reset - надпись над кнопкой,  
	...
	* `dirname` - атрибут для `<input>` и `<textarea>`, который при отправке формы добавляет отдельное поле с направлением введенного текста (`ltr` или `rtl`). <font color="#7ead74">_**[Baseline 2026]**_</font>
...
* `<textarea>` - многострочное поле ввода
	...
	* `wrap` - добавление символов переноса строк при отправке текста с формы _hard(добавляются)/soft(не добавляются)_  
...
* `<button>` - кнопка
	...
	* `type` - типы кнопки (`sumbit, reset, button`) 
...
* `<select>` - выпадающий список.  
	...
	* `multiple` - возможность загрузить больше одного файла (совместим с email, file)  
...
У одного элемента может быть несколько `<label>`, если это нужно по смыслу
...
Если нескольким элементам задан одинаковый `name` - будет оставлен открытым только один блок <font color="#7ead74">_**[Baseline 2024]**_</font>  
```html
...
Значение атрибута `plaintext-only` - элемент остается редактируемым, но при вставке/вводе принимает только обычный текст без rich-text форматирования. <font color="#7ead74">_**[Baseline 2025]**_</font>  
...
* `inert` - атрибут, отключающий интерактивность элемента и всего его subtree. [MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/inert) <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
Тег `<style>` - работает по обычным правилам каскада и специфичности.  
Инлайн стили через атрибут `style=""` имеют больший вес, чем селекторы в файле стилей.  
...
* initial - сброс стиля к начальному значению свойства по спецификации, до изменения стиля разработчиком/браузером 
...
Элементы в одном контексте наложения расположены тем выше, чем они ниже в потоке
...
4. Элемент с `float`
5. Элемент с `display: inline`
6. Элемент (включая потомков) с `z-index` `auto` или 0 или с `opacity: 0`
7. Элемент (включая потомков) с `z-index` больше 0
...
Функция layer() - позволяет расставлять приоритет стилей совместно с подключаемыми библиотеками  
```javascript
...
Через ключевое слово `from` можно создавать новый цвет на основе другого цвета, получая доступ к его каналам (`r`, `g`, `b`, `h`, `s`, `l`, `alpha` и т.д.) и изменяя их. <font color="#7ead74">_**[Baseline 2024]**_</font>  

[Примеры](https://developer.chrome.com/blog/css-relative-color-syntax)

Добавлена поддержка новых цветовых пространств и функций в `color()` / `color-mix()`. <font color="#7ead74">_**[Baseline 2023]**_</font>  
```css
...
* cqw, cqh, cqi, cqb, cqmin, cqmax - единицы измерений, зависящие от размеров контейнера (`@container`) <font color="#7ead74">_**[Baseline 2023]**_</font>  
* lh - мультипликатор, зависящий от `line-height` <font color="#7ead74">_**[Baseline 2023]**_</font>  
```css
/* Было */
--line-height: 1.5;
...
* `:hover` - стиль элемента при наведении курсора (_не работает на тач-устройствах_)  
...
* `child-група` - стиль элементов по порядковому номеру внутри родительского
	...
	* `:nth-last-child(n)` - n-ные дочерний элемент с конца (разрешены формулы с целочисленным результатом)  
    Микросинтаксис формул: `:nth-child(An+B [of S]?)`: <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
* `type-группа` - стиль элементов одного типа по порядковому номеру
	...
	* `:nth-of-type(even)` - четные элементы данного типа
	* `:nth-of-type(odd)` - нечетные элементы данного типа
...
* `:has()` - стиль элемент в зависимости от его потомков или содержимого <font color="#7ead74">_**[Baseline 2023]**_</font>   
...
* `:invalid, :valid` - стиль ошибочно/корректно заполненного поля  
* `:user-valid, :user-invalid` - аналог `:invalid, :valid` с решенной проблемой применения псевдокласса до начала взаимодействия пользователя с элементом <font color="#7ead74">_**[Baseline 2023]**_</font>
...
* `:lang(), :dir()` - стиль элементов по языку/направлению текста (`:dir` <font color="#7ead74">_**[Baseline 2023]**_</font>)
...
* `:read-only` - стиль элементов, недоступных для редактирования (`<input>, <textarea>`, элементы с contenteditable)
* `:active-view-transition` - псевдокласс root-элемента для стилизации страницы, пока выполняется `view transition`. <font color="#7ead74">_**[Baseline 2026]**_</font>
...
Для улучшения доступности можно добавить текстовое описание элемента через `/`.  
Пример: `content: "ⓘ" / "Описание"`  
...
* `::target-text` - псевдоэлемент для стилизации текста, к которому браузер проскроллил страницу через text fragments (`#:~:text=...`) <font color="#7ead74">_**[Baseline 2024]**_</font>
...
* `animation-fill-mode` - применение ключевых кадров до/после проигрывания  
	...
	* backwards - до начала анимации применится первый кадр
	* both - до начала анимации применится первый кадр, после конца последний  
Шорткат для всех свойств анимации: `animation`. Порядок значений не важен. Первое значение времени - длительность.  
К одному элементу можно применять несколько анимаций  
* `animation-composition` - CSS-свойство, определяющее, как несколько анимаций комбинируются, если одновременно влияют на одно и то же свойство <font color="#7ead74">_**[Baseline 2026]**_</font>:
...
Свойство `will-change()` заранее сообщает браузеру какой элемент будет изменяться - повышение производительности.  
* auto - по умолчанию
...
Свойство `display` поддерживает комбинацию двух значений (`outer inner`). <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
Свойства позволяющие работать с дирктивой [@container](#211-директивы) <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
* `content-visibility` - CSS-свойство, позволяющее браузеру пропускать layout/paint/rendering для содержимого элемента. <font color="#7ead74">_**[Baseline 2025]**_</font>   
...
* `round(type, value, interval)` - функция округления <font color="#7ead74">_**[Baseline 2024]**_</font>, где:
...
* тригонометрические функции `sin(), cos(), tan(), asin(), acos(), atan(), atan2()` <font color="#7ead74">_**[Baseline 2023]**_</font>  
* `rem(dividend, divisor)` - остаток от деления со знаком делимого <font color="#7ead74">_**[Baseline 2024]**_</font>  
* `mod(dividend, divisor)` - остаток по модулю со знаком делителя <font color="#7ead74">_**[Baseline 2024]**_</font>  
`mod` - сохраняет знак divisor, `rem` - сохраняет знак dividend
* `abs()`, `sign()` - <font color="#7ead74">_**[Baseline 2025]**_</font>
---
...
* `position` - позиционирование элемента
	...
	* absolute - элемент вырезается из основного потока. Позиционируется относительно ближайшего positioned-родителя (`relative/absolute/fixed/sticky`)  
...
* `inset` - шорткат для `top/right/bottom/left`  
...
* `background-size` - изменение размера фонового изображения  
	...
	* contain - масштабирование с сохранением пропорций так, чтобы изображение вписалось в размер элемента
...
* `backdrop-filter` - применение фильтра (blur, contrast и т.д.) к фоновому изображению под элементом с данным свойством <mark>Baseline 2024</mark>   
...
serif - с засечками, sans-serif - без засечек, monospace - моноширный, cursive - курсивный, fantasy - декоративный, system-ui - системный, math - для отображения математических выражений  
...
* `font-size-adjust` - подстраивает размер fallback-шрифта так, чтобы он выглядел ближе к основному шрифту по выбранной метрике <font color="#7ead74">_**[Baseline 2024]**_</font>  
...
* `text-decoration-style` - стиль декоративной линии (solid, double, dotted, dashed, wavy)
...
* `align-content` - выравнивание рядов флекс-элементов относительно поперечной оси (stretch, flex-start/start, flex-end/end, center, space-between, space-evenly, space-around)  
>Позволяет выравнивать содержимое блочных и табличных контейнеров по вертикали без перевода контейнера в `flex` или `grid` <font color="#7ead74">_**[Baseline 2024]**_</font>  
...
`grid-template-columns: subgrid` и/или `grid-template-rows: subgrid` - наследование размеров родительского grid в дочерних `display:grid` элементах <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
Принимают больше 1 значения (паттерн размеров), которые последовательно повторяются при переполнении грид-сетки  
...
В условиях с числовыми значениями можно использовать операторы сравнения: <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
>`light-dark(lightColor, darkColor)` - функция, выбирающая один из двух цветов в зависимости от текущего `color-scheme` <font color="#7ead74">_**[Baseline 2024]**_</font>
>```css
>/* Было */
>:root {
>  --primary-color: #ccc;
...
* `@container` - условные контрукции в стилях относительно размера контейнера <font color="#7ead74">_**[Baseline 2023]**_</font>  
...
* `@starting-style` - директива для анимирования entry transitions элемента перед его появлением/рендером, в том числе для дискретных свойств (`display`, `visibility`, `mix-blend-mode` и аналогов). <font color="#7ead74">_**[Baseline 2024]**_</font>  
...
`@property <custom-property-name> { syntax, inherits, initial-value }` - директива для регистрации типизированных кастомных CSS-свойств <font color="#7ead74">_**[Baseline 2024]**_</font>, где
...
shape() - аналог `path()`, но с более удобным CSS-синтаксисом (поддержка CSS unit и math function (`calc()`, `max()`, `abs()`)) <font color="#7ead74">_**[Baseline 2026]**_</font>
* `offset-path` - задает путь движения элемента  
* `offset-position` - стартовая позиция элемента для анимации/движения, заданного через `offset-path`. <font color="#7ead74">_**[Baseline 2026]**_</font>
* `scroll-behavior` - управление поведением прокрутки внутри элемента (smooth - плавная, auto - мгновенная)  
Влияет **только** на прокрутку при переходе по якорным ссылкам и на программную прокрутку
* `scroll-padding` - отступ от края контейнера прокрутки (по умолчанию якорный элемент прилипает к верху экрана)
* `scrollbar-color` - цвета thumb и полосы прокрутки (у свойства слабая поддержка, нужно использовать вендорные префиксы) <font color="#7ead74">_**[Baseline 2025]**_</font>  
* `scrollbar-width` - ширина скролл-бара (`auto`, `thin`, `none`) <font color="#7ead74">_**[Baseline 2024]**_</font>  

<details>
  <summary>Пример стилизации скроллбара в tw</summary>

```css
@utility scrollbar {
  --scrollbar-color-thumb: var(--primary);
  --scrollbar-color-thumb-hover: var(--accent);
  --scrollbar-color-track: var(--secondary);
  --scrollbar-width: thin;
  --scrollbar-width-legacy: 0.375rem;
  ...
  @supports selector(::-webkit-scrollbar) {
    ...
    /* Fallback workaround if hover styling breaks in a specific browser/toolchain:
       &:hover {
         --fix: ;
       }
    */
...
* `scrollbar-gutter` - позволяет зарезервировать место для скролбара, при его отсутствии, чтобы избежать сдвига макета <font color="#7ead74">_**[Baseline 2024]**_</font>  
...
* `widows` - минимальное количество строк на новой странице (для @media print)
...
* `counter-reset` - создание счетчика: `counter-reset: timerName 0`
Поддерживает создание нескольких счетчиков
* `counter-increment` - увеличение счетчика на заданную величину: `counter-increment: timerName 2`  
Если указан несуществующий счетчик, то работает как `counter-reset`
* `counter-set` - изменение текущего значения счетчика на указанную: `counter-set: timerName 10` <font color="#7ead74">_**[Baseline 2023]**_</font>  
Если указан несуществующий счетчик, то работает как `counter-reset` <font color="#7ead74">_**[Baseline 2023]**_</font>
...
`@counter-style` - правило для стилизация счетчиков <font color="#7ead74">_**[Baseline 2023]**_</font>  
* `system` - алгоритм построения строкового представления marker representation (`cyclic, numeric, alphabetic, symbolic, additive, fixed, fixed <integer>, extend`)
* `negative` - символы/строки, добавляемые к отрицательным значений счетчика
...
* `symbols` - маркеры счетчика (символ, строка, url) из которых строится представление маркера.
