...
_Термин Virtual DOM устаревшее понятие._
...
```
Чтобы избежать перемонтирования компонента нужно сохранить его позицию в DOM с использованием условного рендеринга или  добавить key, что помогает React правильно сопоставлять элементы между рендерами и сохранить нужный компонент при изменении порядка
```typescript
function Parent(isVisible: boolean) {
	if(isVisible) {
		return (
...
```
**Не стоит** создавать компонент внутри другого компонента, так как при каждом рендере будет создаваться новая функция компонента  
...
2. `const MyComponent = ({prop1, prop2}: PropsWithChildren<CustomPropsType>)` - `CustomPropsType` вместе с `children?: ReactNode | undefined`
...
При отсутствии `React.memo` передача в setState НЕизменившегося состояния `setState(state)` **может пропустить повторный рендер компонента и его дочерних элементов**
...
При создании списков необходимо задавать атрибут `key` с уникальным значением для каждого итерируемого элемента  
...
**React сохраняет состояние компонента, если он НЕ меняет позицию в дереве рендера**  
...
```typescript
const [state, setState] = useState({name: 'Petr', age: 18, id: 1})
return (
	<>
...
**React может пропустить повторный ререндер, если передать неизмененное состояние** `setState(state)`, **НО** вычисления в теле компонента до этого сравнения могут успеть выполниться  
...
handleClick() {
	...
	setHuman({...human, age: 21, name: 'Olga'})
}
```
...
||useReducer|useState|
|----|----|----|
|Тип состояния|Сложные, многосоставные типы данных(Object, Array, Map и т.д.)|Простые типы: (Number, String, Boolean и т.д.)|
...
При передаче ref в **функциональный** компонент, в React 18 и ниже нужно обернуть этот компонент в `forwardRef`  
...
const MyComponent = forwardRef<HTMLInputElement, MyComponentProps>((props, ref) => { 
...
const ChildComponent = forwardRef<MyRef, ChildProps>((props, ref) => {
...
Срабатывает раньше `useLayoutEffect` и `useEffect`. Последовательность: `useInsertionEffect > useLayoutEffect > useEffect`  
