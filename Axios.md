# Содержание
* [1. Запросы](#1-запросы)
	* [1.1 Адаптер и тестирование](#11-адаптер-и-тестирование)
* [2. Ответы](#2-ответы)
* [3. Перехват запросов](#3-перехват-запросов)
* [4. Отмена запроса](#4-отмена-запроса)
* [5. Обработка ошибок](#5-обработка-ошибок)
* [6. Работа с формами](#6-работа-с-формами)
...
axios.defaults.headers.get = {}; // для headers нужно указывать метод запроса (common - все методы)
```
...
* `transformRequest: [function (data, headers) { return data }]` - преобразование данных запроса перед отправкой. **Применимо только** к PUT, POST, PATCH, DELETE  
...
* `data: { name: 'Petr' }` - данные передаваемые в POST, PUT, PATCH, DELETE запросах
	Если отсутствует параметр `transformRequest`, то данные должны быть в форматах: String, Object, ArrayBuffer, ArrayBufferView, URLSearchParams, (FormData, Blob, File - для браузера)  
	Альтернативный синтаксис: `data: 'Name=Petr Petrovich&Age=23'`
* `timeout: 1000` - время ожидания запроса (по умолчанию 0)
* `withCredentials: false` - указание на включение в запрос cookies, аутентификационных заголовков и SSL-сертификатов (по умолчанию false)
...
* `mock.restore()` - восстанавливает оригинальные обработчики запросов axios (прекращение использования мока, выполняются настоящие HTTP-запросы)
* `mock.resetHandlers()` - сбрасывает настройки мока, но mock adapter остается активным и может быть перенастроен. Используется для переопределения мока во время теста
...
* `mock.onGet(url).timeoutOnce()` - аналогично `timeout`, но срабатывает 1 раз. Следующий запрос выдаст мок по запросу, если он настроен
...
`interceptors` позволяют обработать config до/после отправки запроса/ответа. Используются для логирования, добавления заголовков, обработки ошибок и изменения конфига  
_Для исключения повторений в обработке ошибок, можно реализовать ее в interceptors вместо блоков catch каждого запроса_
...
axios.get(url)
	.catch(function (error) {
		if (error.response) { /* запрос вызвал ответ со статусом вне диапазона 2xx */ }
